...
jPlayerAdapter.prototype = {
    ...
    seek: function(t) {
        if (this.jplayer.data().jPlayer.status.paused) {
...
var Util = {
    ...
    fixFlashTag: function() {
        var $ = require("jquery");
        // fix flash tags
        while ($('P[ALIGN]').length > 0) {
            $('P[ALIGN]').replaceWith(function() {
                var tag = $(this);
                return $("<p/>").html(tag.html()).css({
                    "text-align": tag.attr("ALIGN").toLowerCase(),
                    "margin": "0px",
                    "padding": "0px",
                    "border-width": "0px"
                });
        ...
        while ($("FONT").length > 0) {
            $(".StaticText FONT").replaceWith(function() {
                var tag = $(this);
                ...
                return $("<span/>") //create new span
                    ...
                    .css({
                        ...
                        "line-height": parseInt(tag.attr("SIZE")) + parseInt(txtfmt ?
            ...
            $(".ScrollText FONT").replaceWith(function() {
                var tag = $(this);
                var txtfmt = tag.parent("TEXTFORMAT");
                return $("<span/>") //create new span
                    .html(tag.html()) //set html of the new span with what was in font tag
        ...
        while ($('TEXTFORMAT').length > 0) {
            $('TEXTFORMAT').contents().unwrap();
        }
        $(".ScrollText").parent().mCustomScrollbar({
...
var ParsePage = function() {
    ...
    return function(page, slideFrame, zoom) {
        ...
        lastPageTime = end_t;
        var p = new Page(parseInt(page.$.pageIndex), page.$.pageType, start_t, dom);
...
var Dom = {
    ...
    createElement: function(ele, parent) {
        var elebox = $("<div />").css({
                ...
                "height": Util.Unit.dp2px(parseFloat(ele.$.height), 240) + "px"
            })
            .appendTo(parent);
...
HJPlayer = function(settings) {
    ...
    this.layout = {
        rect: { position: "relative", left: 0, top: 0, width: 0, height: 0 }, // 整体矩形区域
        top: { position: "absolute", left: 0, top: 0, width: 0, height: 0 }, // 标题栏
        center: { position: "absolute", left: 0, top: 0, width: 0, height: 0 }, // 中间区域
        slide: { position: "absolute", left: 0, top: 0, width: 0, height: 0 }, // 课件（实际大小可能超出rect，然后css的scale）
        bottom: { position: "absolute", left: 0, top: 0, width: 0, height: 0 } // 控制面板
    }
    this.backup = null;
    this.cache = null;
    this.init();
...
HJPlayer.prototype = {
    ...
    setDefalutLayout: function() {
        ...
        var top_h = $("#jp_container_1 .top-gui").height();
        var bottom_h = $("#jp_container_1 .bottom-gui").height();

        // var screen_w = $("#player").width(); // 居中计算
        // var screen_h = $(window).height();
        $.extend(true, this.layout, {
            rect: {
                left: 0, //(screen_w - slide_w) * 0.5,
                top: 0, //(screen_h - (top_h + slide_h + bottom_h)) * 0.5,
                width: slide_w,
    ...
    scaleLayout: function(scale, origin, s_layout) {
        var layout = s_layout || this.layout; // 旧布局
        // 最大的外框，矩形区域
        ...
        // 中间区域，黑色背景区。不一定符合内容的长宽比例
        var center_w = rect_w;
        var center_h = rect_h - top_h - bottom_h;
        // 按比例放缩内容，填充黑色区域
        var zoom = Math.min(center_w / layout.slide.width, center_h / layout.slide.height);
        var slide_w = layout.slide.width * zoom;
        var slide_h = layout.slide.height * zoom;
        // 更新布局
        $.extend(true, this.layout, {
            rect: {
                left: 0, // layout.rect.left + (layout.rect.width - rect_w) * (origin.x / 100),
                top: 0, // layout.rect.top + (layout.rect.height - rect_h) * (origin.y / 100),
                width: rect_w,
                height: rect_h
            },
            top: { width: rect_w },
            center: { width: center_w, height: center_h },
            slide: { top: (center_h - slide_h) * 0.5, left: (center_w - slide_w) * 0.5, width: slide_w, height: slide_h },
            bottom: { top: rect_h - bottom_h, width: rect_w }
        });
    },
    render: function() {
        var _this = this;
        ...
        if (!this.cache || change(this.layout.rect, this.cache.rect)) {
            ...
            if (!this.cache || sizeChange(this.layout.rect, this.cache.rect)) {
                ...
                $("#jp_container_1 .jp-type-playlist").css({
                    "position": "relative",
                    "width": this.layout.rect.width + "px",
                    "height": this.layout.rect.height + "px",
                    "left": "0px",
                ...
                $('#container #r').height(this.layout.rect.height);
                $('#container #l').height(this.layout.rect.height);
                $('#container #t').width(this.layout.rect.width);
                $('#container #b').width(this.layout.rect.width);
            }
        }
    ...
    remoteLoad: function() {
        ...
        $.ajax({
        	type: "GET",
        	url: _this.settings.dataPath + _this.settings.slidesData + "?=t" + $.now(),
        	dataType: "xml",
        	success: function(xmlDoc) {
        		var x2js = new X2JS();
        		var xml =xmlparser.toJson(xmlDoc);
        		_this.parseData(xml);
        	}
        });*/
    },
	...
	if (page._pageType == "0") {
      ...
      if (page.ele._type == "1") { // 选择题
		...
		*/
        ...
        Util.fixFlashTag();
        $(".background").removeClass("defaultscreen");
        this.audioPlayer.setDataSource(_this.settings.dataPath + _this.settings.audioFileName);
    ...
    updatePageNumnber: function(index) {
        $('#jp_container_1 .jp-page-number').html(index + "/" + this.pageCount);
    },
    updateTimeDuration: function(t) {
        $('#jp_container_1 .jp-current-duration').html(Util.Time.format(t) + "/" + Util.Time.format(this.duration));
    },
    ...
    fullscreen: function() {
        $(".jp-full-screen").click(); // depends on jplayer, how to make independent ?
    },
    restore: function() {
    ...
    isFullScreen: function() {
        ...
        return document.webkitIsFullScreen || document.mozFullScreen || document.fullscreenElement || document.msFullscreenElement;
    },
    bindUpdate: function() {
        var _this = this;
    ...
    bindClick: function() {
        var _this = this;
        $(".trim-space").off("mousedown click").on('mousedown', function(e) {
            $(this).data("initcoords", { x: e.x, y: e.y });
        }).on("click", function(e) {
    ...
    bindCustomButton: function() {
        var _this = this;
        $("#jp_container_1 .jp-next").unbind("click").bind("click", function() {
            _this.gotoNextPage();
        });
        $("#jp_container_1 .jp-previous").unbind("click").bind("click", function() {
            _this.gotoPrevPage();
        });
    },
    /*
    bindDrag: function() {
        ...
        $("#container").off("mousedown", "#title").on('mousedown', '#title', function(e) {
            var offset = target.offset();
            var posix = { 'x': e.pageX, 'y': e.pageY };
            $.extend(document, {
                'move': true,
                'move_target': target,
    ...
    bindReszie: function() {
        var _this = this;
        $("#container").off("mousedown", '#l, #r, #t, #b, #lt, #rt, #lb, #rb')
            .on('mousedown', '#l, #r, #t, #b, #lt, #rt, #lb, #rb', function(e) {
                ...
                var transform = function(e) {
                    var scale = {
                        x: (opt[0] * (e.pageX - posix.x) + layout.rect.width) / layout.rect.width,
                        y: (opt[1] * (e.pageY - posix.y) + layout.rect.height) / layout.rect.height
                    };
                    var origin = { x: opt[2], y: opt[3] };
        ...
        if (opt == "hide") {
            switch (controls) {
                case "openfile":
                    ...
                    $(".file").parent().hide();
                    $(".jp-previous").css("left", "10px");
                    $(".jp-play").css("left", "50px");
                    $(".jp-pause").css("left", "50px");
                    $(".jp-next").css("left", "100px");
                    $(".jp-page-number").css("left", "155px");
                    break;
                default:
                    ;
            }
            ...
            switch (controls) {
                case "current-duration":
                    var full = $(".jp-full-screen").is(":visible");
                    var volume = $(".jp-volume-bar").is(":visible");
                    if (!full && !volume) {
                        $(".jp-current-duration").css({
                            "right": "12px"
                        });
                    } else if (!volume) {
                        $(".jp-current-duration").css({
